...
#import "DZCHelp.h"
@interface WMPageController () <WMMenuViewDelegate,UIScrollViewDelegate,UISearchBarDelegate> {
    CGFloat viewHeight;
    CGFloat viewWidth;
    CGFloat targetX;
    BOOL    animate;
...
- (void)viewDidLoad {
    ...
    [self AddSearchBar];
    UIBarButtonItem *leftItem=[[UIBarButtonItem alloc]initWithTitle:@"返回" style:UIBarButtonItemStylePlain target:self action:@selector(dzcBackItem)];
...
//                                                           [UIFont fontWithName:@"HelveticaNeue-CondensedBlack" size:20], NSFontAttributeName, nil]];
...
{
    LFSearchViewController *searchView=[[LFSearchViewController alloc]init
                                        ];
    UINavigationController *nav=[[LFNavigationController alloc]initWithRootViewController:searchView];
    
    [self presentViewController:nav animated:NO completion:^{
        [searchBar resignFirstResponder];//搜索栏恢复原来状态
    }];
    //[searchBar resignFirstResponder];//搜索栏恢复原来状态
}
